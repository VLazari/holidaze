			...
			{props.rating < 1 ? (
				...
				<>
					{[1, 2, 3, 4, 5].map((rating) =>
						props.rating >= rating ? (
							<FontAwesomeIcon key={rating} className="mx-0.5 text-sm text-gold-main" icon={faStar} />
						) : props.rating < rating && props.rating > rating - 1 ? (
							<FontAwesomeIcon key={rating} className="mx-0.5 text-sm text-gold-main" icon={faStarHalfStroke} />
						) : (
