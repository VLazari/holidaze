...
export default function BookingCard({ bookings }) {
	...
	const deleteBooking = async (id) => {
		const response = await apiDelete(`https://api.noroff.dev/api/v1/holidaze/bookings/${id}`);
		response === 204 && setUpdatedBookings((prevBookings) => prevBookings.filter((booking) => booking.id !== id));
	};
	...
	return (
		<div className="flex flex-col bg-white shadow-md">
			<div className="flex-1 px-2 py-6 sm:px-6">
				...
				{updatedBookings.length === 0 && <h3 className="m-5 text-md font-medium text-gray-500">You don't have any booked venues</h3>}
				<div className="mt-8">
					<ul role="list" className="divide-y divide-gray-200">
						{updatedBookings.map((booking) => {
							const totalPrice = getNumberOfNights(new Date(booking.dateFrom), new Date(booking.dateTo)) * booking.venue.price;
							return (
									...
									<div className="mx-1 sm:mx-3 flex w-full sm:flex-1 flex-col">
										<div className="py-3 sm:p-0">
											<div className="flex justify-between text-base font-medium text-gray-900">
												<h3>
													<Link to={`/venue/${booking.venue.id}`}>{booking.venue.name}</Link>
												</h3>
