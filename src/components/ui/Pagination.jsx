			...
			<div className="flex flex-col-reverse items-center">
				<div>
					<p className="text-sm text-gray-700">
						Showing <span className="font-medium">{currentPage === 1 ? 1 : startIndex + 1}</span> to{" "}
						<span className="font-medium">{endIndex > venues.length ? venues.length : endIndex}</span> of{" "}
						<span className="font-medium">{venues.length}</span> results
					</p>
				</div>
				<div>
					<nav className="isolate inline-flex flex-wrap justify-center -space-x-px rounded-md shadow-sm" aria-label="Pagination">
						<div
						...
						<div
							onClick={() => currentPage < nrOfPages && setCurrentPage(currentPage + 1)}
