...
export default function BookingData({ venue }) {
  ...
  if (response && response.id) {
    return (
      <section className="mt-5 p-10 border ">
        <h3 className="pb-4 text-green-500 text-center text-2xl font-semibold">
          Booking successful
        </h3>
        ...
        <p className="py-1 mx-1 text-md font-semibold tracking-tight text-blue-main">
          To: {format(new Date(response.dateTo), 'dd/MM/yyyy')}
        </p>
        <p className="pt-3 text-xl font-semibold tracking-tight text-blue-main">
          <span className="text-md text-gray-500">Total payed: </span> $
          {totalPrice}
        </p>
        <Link
  ...
  return (
    <section className="mt-5 p-10 border ">
      <h3 className="pb-6 text-blue-main text-lg text-center font-semibold">
      ...
      />
      <p className={`${showError} text-xs text-red-600 italic`}>
        This dates are not available
      </p>
      <div className="mx-auto my-8 md:mx-0">
        <span className="text-gray-500 select-none mr-5">
